...
class AMSSolver:
    ...
    def AMSComputeGPi(self):
        ...
        data[0::2] = -1.0
        data[1::2] =  1.0
        self.G = csr2((data, (row, col)), shape=(num_edges, num_nodes))
        print(type(self.A))
    ...
    def AMSComputePi(self):
        ...
        self.Pi = csr2((bm.array(data), (bm.array(rows), bm.array(cols))), shape=(num_edges, dim*num_nodes))
...
import pyamg
from fealpy.sparse import CSRTensor as csr
from fealpy.sparse import csr_matrix as csr2
...
class ASPForPoisson(object):
    ...
    def generate_preconditioner(self):
        ...
        def mix_preconditioner(r1, P0, P1, PI, A):
            ...
            zc = P1.AMSSolve(PI.T @ r1)  # P1.AMSSolve SHOULD now return x (1D)
                # 兼容旧版本（若 P1.AMSSolve 仍返回 tuple/list）
            if isinstance(zc, (tuple, list)):
                    zc = zc[0]
            e1  = PI@(zc)
            e1 += 0.5*P0@r1
            return e1 
