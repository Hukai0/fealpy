...
from .sparse_core import csr_utils
...
class CSRTensor(SparseTensor):
    ...
    #     crow, col, values = csr_utils.csr_transpose(self._crow, self._col, self._values, self._spshape[0], self._spshape[1])
    #     return CSRTensor(crow, col, values, spshape=(self._spshape[1], self._spshape[0]))
    ...
    def find(self):
        ...
        row, col, values = csr_utils.csr_find_nonzero(self._crow, self._col, self._values)
    ...
    # def col_min(self):
    #     from .sparse_core import csr_utils
    #     return csr_utils.csr_column_min(self._crow, self._col, self._values, self._spshape[0], self._spshape[1])
    ...
    def __getitem__(self, index):
        ...
        if col_index is not None:
            if isinstance(col_index, slice):
                ...
                col_index = bm.array([col_index], dtype=bm.int64)
            elif isinstance(col_index, (List, TensorLike)):
                col_index = bm.array(col_index, dtype=bm.int64)
            else:
                raise TypeError(f'index must be a slice, int, or list, but got {type(index)}')

        if isinstance(crow_index, slice):
            start = crow_index.start if crow_index.start is not None else 0
            stop = crow_index.stop if crow_index.stop is not None else self._spshape[0]
            step = crow_index.step if crow_index.step is not None else 1
            crow_index = bm.arange(start, stop, step)
        elif isinstance(crow_index, int):
            crow_index = bm.array([crow_index], dtype=bm.int64)
        elif isinstance(crow_index, (List, TensorLike)):
            crow_index = bm.array(crow_index, dtype=bm.int64)
        else:
            raise TypeError(f'index must be a slice, int, or list, but got {type(index)}')

        # 直接调用 csr_extract_submatrix
        new_crow, new_col, new_values = csr_utils.csr_extract_submatrix(self._crow, self._col, self._values, crow_index, col_index)

        return CSRTensor(new_crow, new_col, new_values, spshape=(len(crow_index), len(col_index)))
# 原来的实现
