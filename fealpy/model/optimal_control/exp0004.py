...
class Exp0004(BoxMesher2d):
    ...
    def __init__(self, options: dict = {}):
        ...
        self.y = phi * t                # y(x,t)
        self.z = -4*phi * (1 - t)                  # z(x,t)
        # self.u = -self.z                     # z(x,t)
        self.u = 4*phi * (1 - (t) )
        # self.u = bm.maximum(1, bm.minimum(3 ,-self.z))                            # u(x,t) = -z

        # flux p 
        self.p0 = -2 * x1 * (1 - x1) * x2**2 * (1 - 2 * x1)*(1-x2)**2 * t  
        self.p1 =  -2 * x2 * (1 - x2) * x1**2 * (1 - 2 * x2)*(1-x1)**2 * t  
        self.p = sp.Matrix([self.p0, self.p1])

        # adjoint flux q
        self.q0 = 8 * x1 * (1 - x1) * x2**2 * (1 - 2 * x1)*(1-x2)**2 * (1 - t)  
        self.q1 =  8 * x2 * (1 - x2) * x1**2 * (1 - 2 * x2)*(1-x1)**2 * (1 - t)  
    ...
    def y_solution(self, space, time):
        ...
        x2 = self.x2
        t = self.t
        func = sp.lambdify([x1, x2, t], self.y, self.manager)
        x = space[..., 0]; y = space[..., 1]
    ...
    def z_solution(self, space, time):
        ...
        x2 = self.x2
        t = self.t
        func = sp.lambdify([x1, x2, t], self.z, self.manager)
        x = space[..., 0]; y = space[..., 1]
        return func(x, y, time)
    ...
    def u_solution(self, space, time):
        ...
        x2 = self.x2
        t = self.t
        func = sp.lambdify([x1, x2, t], self.u, self.manager)
        x = space[..., 0]; y = space[..., 1]
        return func(x, y, time)
    ...
    def f_fun(self, space, time):
        ...
        func = sp.lambdify([x1, x2, t], self.f, self.manager)
        x = space[..., 0]; y = space[..., 1]
        return func(x, y, time)
